...
const DrawingInfoRenderer = () => {
  ...
  const info = useMemo(() => buildDrawingInfo(shapes, unit, ppm), [shapes, unit, ppm]);
  ...
  return (
    <Group x={tableX} y={tableY} listening={false}>
      {/* Outer border */}
      <Rect x={0} y={0} width={TOTAL_W} height={totalH} fill={ROW_EVEN} stroke={BORDER} strokeWidth={1} listening={false} />
      ...
      {COLS.reduce<{ x: number; els: React.ReactNode[] }>(
        (acc, col, i) => {
          acc.els.push(
            <Text key={`hdr-${i}`} x={acc.x + PAD} y={(HEADER_H - HEADER_FONT_SZ) / 2} text={col.label}
              fontSize={HEADER_FONT_SZ} fontFamily="monospace" fontStyle="bold" fill={HEADER_FG} listening={false} />,
          );
          if (i < COLS.length - 1)
            acc.els.push(<Line key={`hv-${i}`} points={[acc.x + col.w, 0, acc.x + col.w, HEADER_H]} stroke="#334155" strokeWidth={0.5} listening={false} />);
          acc.x += col.w;
          return acc;
        },
        { x: 0, els: [] },
      ...
      {info.rows.map((row, ri) => {
        ...
        return (
          <Group key={ri} listening={false}>
            ...
            {COLS.map((col, ci) => {
              const el = (
                <Group key={ci} listening={false}>
                  <Text x={cx + PAD} y={ry + (ROW_H - FONT_SZ) / 2} text={cells[ci]}
                    fontSize={FONT_SZ} fontFamily="monospace" fill={TEXT_COLOR}
                    width={col.w - PAD * 2} ellipsis listening={false} />
                  {ci < COLS.length - 1 && (
                    <Line points={[cx + col.w, ry, cx + col.w, ry + ROW_H]} stroke={BORDER} strokeWidth={0.5} opacity={0.4} listening={false} />
                  )}
                </Group>
              );
              cx += col.w;
            ...
            <Line points={[0, ry + ROW_H, TOTAL_W, ry + ROW_H]} stroke={BORDER} strokeWidth={0.5} opacity={0.3} listening={false} />
          </Group>
        );
      })}
    </Group>
