...
const AdminEstimateSection = () => {
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      {/* Mode: a single assembly sandbox, or the live drawing. */}
      <div className="inline-flex rounded-md bg-panel-2 p-0.5 hair">
        {(["assembly", "drawing"] as const).map((m) => (
          <button
            key={m}
            type="button"
            onClick={() => setMode(m)}
            className={cn(
              ...
              mode === m ? "bg-brand text-brand-foreground" : "text-ink-2 hover:text-ink",
            )}
          >
            {t(m === "assembly" ? "admin.singleAssembly" : "admin.currentDrawing")}
          </button>
        ))}
      </div>
